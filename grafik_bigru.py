...
def buat_grafik_bigru_multimodal(df, list_prediksi, hari_target, warna_garis):
    ...
    # BiGRU Projection Line
    fig.add_trace(go.Scatter(
        x=tanggal_full[len_hist-1:], 
        y=emas_idx[len_hist-1:], 
        mode='lines+markers', 
        name='BiGRU Projection', 
        line=dict(color=warna_garis, width=4, dash='dot'), 
        customdata=emas_full[len_hist-1:], 
        hovertemplate="BiGRU Prediction: IDR %{customdata:,.0f}<extra></extra>"
    ))

    # Multimodal Indicator Lines
    fig.add_trace(go.Scatter(
        x=tanggal_full[:len_hist], 
        y=kurs_idx, 
        mode='lines', 
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="Oil: IDR %{customdata:,.0f}<extra></extra>"
    ))
