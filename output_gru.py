...
def tampilkan_hasil_gru(hari_target, model, scaler, df):
  ...
  with st.spinner(
      f"GRU Engine is calculating projections for {tanggal_target}..."
  ):
    (
        skor,
        keputusan,
        ...
        selisih,
        margin,
    ) = evaluasi_spk_gru(hari_target, model, scaler, df)

    # 1. TITLE (Responsive Font Size)
    st.markdown(
        f"<div style='text-align: center; font-size: clamp(20px, 4vw, 28px);"
        " font-weight: bold; margin-bottom: 10px;'>📊 GRU DECISION RESULTS"
    ...
    html_metrik = f"""
        ...
        ">
            ...
            <div style="text-align: center; flex: 1 1 250px; padding: 8px 0;">
                <p style="margin:0; font-size: 12px; color: #6b7280; font-weight: 700; letter-spacing: 0.5px;">GRU AI PROJECTION</p>
    ...
    with col1:
      with st.container(border=True):
        st.markdown(
            f"**{indikator['ai']['icon']} GRU AI Trend**",
            help="Prediksi pergerakan harga oleh model GRU",
        )
        st.metric(
    ...
    with st.spinner("Rendering charts..."):
      list_prediksi_gru = hitung_sekuens_prediksi_gru(
          hari_target, model, scaler, df
      )
      fig_gru = buat_grafik_gru_multimodal(
          df, list_prediksi_gru, hari_target, bg_color
      )
