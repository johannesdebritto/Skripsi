...
def _render_decision_banner(keputusan: str, warna: str):
    """Komponen Banner Keputusan Utama XGBoost."""
    ...
    st.markdown(
        ...
        <p style="text-align: center; color: #888888; font-size: 12px; margin-bottom: 20px;">
            ⚠️ <b>Note:</b> Transaction decisions are entirely at your own discretion. This AI serves as a probabilistic forecasting tool, not a guarantee or financial advice.
        </p>
        """,
...
def _render_metrics_banner(tanggal_sekarang_str: str, h_sekarang: float, h_prediksi: float, selisih: float, margin: float):
    ...
    ">
        ...
        <div style="text-align: center; padding: 4px; border-left: 1px solid rgba(0,0,0,0.05); border-right: 1px solid rgba(0,0,0,0.05);">
            <span style="font-size: 11px; color: #6b7280; font-weight: 700; letter-spacing: 0.5px; display: block;">XGBOOST AI PROJECTION</span>
...
def _render_indicator_cards(indikator: dict):
    ...
    with c1:
        with st.container(border=True):
            st.markdown(f"**{indikator['ai']['icon']} XGBoost AI Trend**", help="Prediksi pergerakan harga oleh model XGBoost")
...
def tampilkan_hasil_xgb(hari_target, model, scaler, df):
    ...
    with st.spinner(f"XGBoost Engine is calculating projections for {tanggal_target}..."):
        ...
        # Header Title
        st.markdown(f"<h3 style='text-align: center;'>📊 XGBOOST DECISION RESULTS ({tanggal_target})</h3>", unsafe_allow_html=True)
        st.divider()
        ...
        with st.spinner("Synthesizing decision rationale based on multivariate data..."):
            sentimen = (
                "BULLISH (Strongly Positive)" if skor >= 2
                else "BEARISH (Strongly Negative)" if skor <= -2
        ...
        with st.spinner("Processing XGBoost multimodal chart..."):
            list_prediksi_xgb = hitung_sekuens_prediksi_xgb(hari_target, model, scaler, df)
            fig_xgb = buat_grafik_xgb_multimodal(df, list_prediksi_xgb, hari_target, bg_color)
