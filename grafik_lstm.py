...
def buat_grafik_multimodal(df, list_prediksi, hari_target, warna_garis):
  # 1. Prepare Historical Data (last 60 days)
  ...
  # 2. Combine Historical + Projected Gold Data
  ...
  # 3. Create Chart Canvas
  fig = go.Figure()

  # --- A. GOLD LINES (Historical & Projection) ---
  ...
  fig.add_trace(
      go.Scatter(
          ...
          mode='lines+markers',
          name='Gold Projection',
          line=dict(color=warna_garis, width=3, dash='dot'),
          marker=dict(size=5),
          customdata=emas_full[len_hist - 1 :],
          hovertemplate=(
              'Gold Projection: IDR %{customdata:,.0f}<extra></extra>'
          ),
      )
  )

  # --- B. MACROECONOMIC LINES ---
  fig.add_trace(
      go.Scatter(
          x=tanggal_full[:len_hist],
          y=kurs_idx,
  ...
  fig.update_layout(
      title={
          'text': '<b>Chart: Evaluation vs Projection</b>',
          'y': 0.96,
          'x': 0.5,
          'xanchor': 'center',
          'yanchor': 'top',
