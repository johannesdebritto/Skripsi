...
def _get_target_and_current_date(df: pd.DataFrame, hari_target: int):
    ...
    try:
        col_date = next((col for col in ["Tanggal", "Date"] if col in df.columns), None)
        tanggal_terakhir = (
            pd.to_datetime(df[col_date].iloc[-1])
            if col_date
...
def _render_decision_banner(keputusan: str, warna: str):
    """Komponen Banner Keputusan Utama."""
    bg_colors = {"success": "#28a745", "error": "#dc3545"}
    bg_color = bg_colors.get(warna, "#ffb300")
...
def _render_metrics_banner(tanggal_sekarang_str: str, h_sekarang: float, h_prediksi: float, selisih: float, margin: float):
    ...
    ">
        ...
        <div style="text-align: center; padding: 4px; border-left: 1px solid rgba(0,0,0,0.05); border-right: 1px solid rgba(0,0,0,0.05);">
            <span style="font-size: 11px; color: #6b7280; font-weight: 700; letter-spacing: 0.5px; display: block;">LSTM AI PROJECTION</span>
...
def _render_indicator_cards(indikator: dict):
    ...
    with c1:
        with st.container(border=True):
            st.markdown(f"**{indikator['ai']['icon']} LSTM AI Trend**", help="Prediksi pergerakan harga oleh model LSTM")
...
def tampilkan_hasil_lstm(hari_target, model, scaler, df):
    ...
    with st.spinner(f"LSTM Engine is calculating projections for {tanggal_target}..."):
        ...
        # Header Title
        st.markdown(f"<h3 style='text-align: center;'>📊 LSTM DECISION RESULTS ({tanggal_target})</h3>", unsafe_allow_html=True)
        st.divider()
